"""
WEBP Convertor engine — ToolCEO
==================================

Convert WEBP images to other formats using Pillow (raster formats),
ImageMagick (ICO), and pytesseract (OCR → TXT).

Public API
----------
convert_webp(data, target_format, job_id) -> bytes
    Convert raw WEBP bytes to *target_format*.
...
-----------------
  jpg   → JPEG (quality 95)
  png   → PNG  (optimize, compress_level=9)
...
def _open_webp(data: bytes):
    """Open WEBP bytes as a Pillow Image."""
    try:
...
def _to_jpg(data: bytes, job_id: Optional[str]) -> bytes:
    ...
    img = _open_webp(data)
    _report(job_id, 50)
    # WEBP may have alpha — JPEG does not support it
...
def _to_tiff(data: bytes, job_id: Optional[str]) -> bytes:
    ...
    img = _open_webp(data)
    _report(job_id, 50)
    # Use LZW compression with horizontal-differencing predictor (tag 317, value 2).
    # This is fully lossless and universally supported, but far smaller than the
    # default uncompressed TIFF that Pillow produces when no compression is given.
    # predictor=2 (horizontal differencing) works well for photographic content
    # and is the standard pair with LZW/Deflate in TIFF files.
    out = io.BytesIO()
...
def _to_ico(data: bytes, job_id: Optional[str]) -> bytes:
    """
    Convert WEBP → multi-resolution ICO using ImageMagick (primary path)
...
def _to_ico_magick(data: bytes, job_id: Optional[str], magick: str) -> bytes:
    ...
    try:
        src  = os.path.join(tmp_dir, "input.webp")
...
# ---------------------------------------------------------------------------
# Public: merge multiple WEBPs into one multi-page PDF
# ---------------------------------------------------------------------------

def merge_webps_to_pdf(
    items:  list[tuple[bytes, str]],
    job_id: Optional[str] = None,
) -> bytes:
    """Combine multiple WEBP images into a single multi-page PDF."""
    _report(job_id, 5)
    n = len(items)
    if n == 0:
...
def convert_webp(
    ...
    """
    Convert raw WEBP *data* to *target_format*.

    Parameters
    ----------
    data          : raw bytes of the source WEBP image
    target_format : one of "jpg", "png", "pdf", "bmp", "tiff", "ico", "txt"
