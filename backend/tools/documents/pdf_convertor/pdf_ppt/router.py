"""
PDF → PPT router — ToolCEO
============================

Endpoints
---------
POST /api/pdf/ppt/info
...
POST /api/pdf/ppt/convert
    Async job.  Accepts a single PDF and enqueues the PDF→PPTX conversion.
    ...
    output_filename  : str | None  – desired filename (default "<stem>_converted.pptx")
"""
...
def _submit(job_id: str, fn, *args, filename: str, media_type: str = _PPTX_TYPE) -> None:
    _pool.submit(_run_job, job_id, fn, *args, filename=filename, media_type=media_type)


# ---------------------------------------------------------------------------
# POST /api/pdf/ppt/info  –  synchronous preview helper
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# POST /api/pdf/ppt/convert  –  async conversion job
# ---------------------------------------------------------------------------

@router.post(
    "/convert",
    summary="Convert a PDF to a PowerPoint presentation (.pptx)",
)
async def pdf_to_ppt(
    ...
    out_name = (output_filename or "").strip() or f"{stem}_converted"
