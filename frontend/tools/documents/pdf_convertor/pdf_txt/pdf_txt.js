/**
 * tools/documents/pdf_convertor/pdf_txt/pdf_txt.js
 *
 * PDF → TXT Converter — single-file flow.
 ...
 *
 * Exports:
 *   handlePdfTxtFilePicked(file)  – call when a file is chosen
 *   removePdfTxtPanel()           – teardown on tool change / reset
 */
...
let _pdfTxtFile     = null;
let _pdfTxtBaseName = '';
let _pdfTxtPageCount = 1;
...
export function removePdfTxtPanel() {
  const panel = document.getElementById('pdf-txt-settings-panel');
  if (panel) panel.remove();

  const zone = document.getElementById('drop-zone');
  if (zone) {
    const thumb = zone.querySelector('.dz-pdf-txt-thumb-wrap');
    if (thumb) thumb.remove();
    zone.classList.remove('dz-has-pdf-txt-thumb');
  }
...
function _showThumb(zone, file, color, dataUri) {
  const old = zone.querySelector('.dz-pdf-txt-thumb-wrap');
  if (old) old.remove();

  const thumbContent = dataUri
    ? `<img class="dz-pdf-txt-thumb-img" src="${dataUri}"
             alt="PDF preview" draggable="false" />`
    : `<svg class="dz-pdf-txt-thumb-icon" viewBox="0 0 90 116"
  ...
  wrap.className = 'dz-pdf-txt-thumb-wrap';
  wrap.innerHTML = `
    <div class="dz-pdf-txt-thumb-card">
      <div class="dz-pdf-txt-thumb-frame" style="border:2px solid ${color};box-shadow:0 4px 18px rgba(0,0,0,0.45)">
        ${thumbContent}
      </div>
      <button class="dz-pdf-txt-thumb-remove" title="Remove file"
              style="--pw-color:${color}" aria-label="Remove file">&#x2715;</button>
    </div>
    <span class="dz-pdf-txt-thumb-name">${_esc(file.name)}</span>
    <span class="dz-pdf-txt-thumb-size">${_fmt(file.size)}</span>`;

  zone.classList.add('dz-has-pdf-txt-thumb');
  zone.appendChild(wrap);

  wrap.querySelector('.dz-pdf-txt-thumb-remove').addEventListener('click', (e) => {
    e.stopPropagation();
    removePdfTxtPanel();
...
function _showSettingsPanel(pageCount, fileSize, color) {
  const existing = document.getElementById('pdf-txt-settings-panel');
  ...
  panel.id        = 'pdf-txt-settings-panel';
  ...
  panel.innerHTML = `
    ...
    <div class="pw-header">
      <span class="pw-header-badge">
        <svg width="14" height="14" viewBox="0 0 16 16" fill="none" aria-hidden="true">
          ...
          <line x1="10" y1="5" x2="15" y2="5" stroke="${color}" stroke-width="1.3" stroke-linecap="round"/>
          <line x1="10" y1="7.5" x2="15" y2="7.5" stroke="${color}" stroke-width="1.1" stroke-linecap="round"/>
          <line x1="10" y1="10" x2="13" y2="10" stroke="${color}" stroke-width="1.1" stroke-linecap="round"/>
        </svg>
      ...
      <button class="pw-change-btn" id="pw-txt-change-btn" title="Pick a different file">
        Change file
      </button>
    </div>
    ...
    <div class="pw-info-box">
      ...
      <span>Extracts all text as a plain UTF-8 .txt file. Multi-column layouts and
      reading order are preserved. Scanned pages are processed via OCR if available.</span>
    </div>

    <!-- ── ACTIONS ROW ──────────────────────────────────────────────── -->
    <div class="pw-actions">
      <input class="pw-filename-input" id="pw-txt-filename-input"
             type="text" placeholder="Output filename (optional)"
             maxlength="120" spellcheck="false"/>
      <span class="pw-filename-ext">.txt</span>
      <button class="pw-submit-btn" id="pw-txt-submit-btn">Extract to TXT</button>
    </div>`;
  ...
  // ── Default filename ──────────────────────────────────────────────────────
  const filenameInput = panel.querySelector('#pw-txt-filename-input');
  if (filenameInput && _pdfTxtBaseName) {
    filenameInput.value = `${_pdfTxtBaseName}`;
  }

  // ── "Change file" button ──────────────────────────────────────────────────
  panel.querySelector('#pw-txt-change-btn').addEventListener('click', () => {
  ...
  // ── "Extract to TXT" button ───────────────────────────────────────────────
  panel.querySelector('#pw-txt-submit-btn').addEventListener('click', () => {
    if (!_pdfTxtFile) return;
    const nameEl  = panel.querySelector('#pw-txt-filename-input');
    const outName = (nameEl ? nameEl.value.trim() : '') || _pdfTxtBaseName;
    const main    = document.getElementById('main-content');
    if (main) main.scrollTop = 0;
    _submitConvert(_pdfTxtFile, outName);
  });
}

// ─── SCAN FLOW ────────────────────────────────────────────────────────────────

export async function handlePdfTxtFilePicked(file) {
  ...
  try {
    const res = await fetch(`${BACKEND}/api/pdf/txt/info`, { method: 'POST', body: fd }).catch(() => null);
    if (res && res.ok) {
  ...
  _pdfTxtFile      = file;
  _pdfTxtBaseName  = file.name.replace(/\.[^.]+$/, '');
  _pdfTxtPageCount = pageCount;
...
async function _submitConvert(file, outputFilename) {
  ...
  const panel = document.getElementById('pdf-txt-settings-panel');
  if (panel) panel.remove();
  const thumb = zone ? zone.querySelector('.dz-pdf-txt-thumb-wrap') : null;
  if (thumb) thumb.remove();
  if (zone)  zone.classList.remove('dz-has-pdf-txt-thumb');

  const fd = new FormData();
  fd.append('file', file);
  fd.append('output_filename', outputFilename);

  showProgress(zone, 9, color, 'Extracting…');

  const earlyFilename = `${_pdfTxtBaseName || outputFilename}.txt`;
  setBgJob({ jobId: null, tool, filename: earlyFilename, progress: 5, state: 'submitting', sse: null });

  let jobId;
  try {
    const res  = await fetch(`${BACKEND}/api/pdf/txt/convert`, { method: 'POST', body: fd });
    const json = await res.json();
    if (!res.ok) {
  ...
  sse.onmessage = (event) => {
    ...
    if (state === 'error') {
      showError(document.getElementById('drop-zone'), error || 'Extraction failed. Please try again.');
    }
  };
