...
function _renderStrip() {
  ...
  // Reorder banner — shown when png-pdf + single + 2+ images
  ...
  if (isPdfSingle) {
    _initPngDragReorder(strip, color);
  }
}
...
// ─── PNG-PDF DRAG-TO-REORDER ──────────────────────────────────────────────────

function _initPngDragReorder(strip, color) {
  let _dragSrcIdx     = -1;
  ...
  function _lockMainScroll() {
    ...
    if (!mc._pngScrollLockBound) {
      mc._pngScrollLockBound = true;
