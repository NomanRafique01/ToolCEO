 ...
 * Mirrors images_pdf.js: users drop / select one or more JPGs,
 * thumbnail cards appear, they can add more, then click Convert.
 *
 * Single file  → backend returns the converted file directly.
 * Multiple files → backend converts all and returns a .zip archive.
 *
 * Exports:
 *   handleJpgFilesPicked(files, toolId)   – entry point from thin wrappers
 *   removeJpgPanel()                       – full teardown
 */
...
let _queue   = [];
let _toolId  = '';
/** 'single' | 'individual' — only relevant for jpg-pdf with multiple images */
...
export function removeJpgPanel() {
  document.getElementById('jpg-queue-panel')?.remove();

  const zone = document.getElementById('drop-zone');
  if (zone) {
    zone.querySelector('.dz-jpg-thumb-strip')?.remove();
    zone.querySelector('.dz-jpg-reorder-banner')?.remove();
    zone.classList.remove('dz-has-jpg-thumbs');
  }
...
function _renderStrip() {
  ...
  zone.classList.add('dz-has-jpg-thumbs');

  // Reorder banner — shown when jpg-pdf + single + 2+ images
  const isPdfSingle = _toolId === 'jpg-pdf' && _pdfMode === 'single' && _queue.length > 1;
  if (isPdfSingle) {
    const banner = document.createElement('div');
    banner.className = 'dz-jpg-reorder-banner';
  ...
  strip.className = 'dz-jpg-thumb-strip';
  ...
  addBtn.type  = 'button';
  addBtn.title = 'Add more JPG images';
  ...
  if (isPdfSingle) {
    _initJpgDragReorder(strip, color);
  }
}
...
function _buildCard(item, idx, color) {
  ...
  const isPdfSingle = _toolId === 'jpg-pdf' && _pdfMode === 'single';
  if (isPdfSingle) {
    card.draggable = true;
...
// ─── JPG-PDF DRAG-TO-REORDER ──────────────────────────────────────────────────

function _initJpgDragReorder(strip, color) {
  let _dragSrcIdx      = -1;
  ...
  function _lockMainScroll() {
    ...
    if (!mc._jpgScrollLockBound) {
      mc._jpgScrollLockBound = true;
...
function _openFilePicker() {
  // Reuse the main file-input if available, else create a temporary one
  ...
  inp.multiple = true;
  inp.accept   = 'image/jpeg,.jpg,.jpeg';
...
function _renderPanel() {
  ...
  const isPdf    = _toolId === 'jpg-pdf';
  const isMulti  = total > 1;

  // For jpg-pdf with multiple files, hint changes based on _pdfMode
  let outHint;
  if (isPdf && isMulti) {
  ...
  // ICO info row — only shown for jpg-ico
  const icoInfoHTML = (_toolId === 'jpg-ico') ? `
  ...
  // PDF mode toggle — only shown for jpg-pdf with 2+ images
  const pdfToggleHTML = (isPdf && isMulti) ? `
    <div class="jpg-pdf-mode-row" id="jpg-pdf-mode-row">
      ...
      <div class="jpg-pdf-mode-toggle">
        <button class="jpg-pdf-mode-btn${_pdfMode === 'individual' ? ' jpg-pdf-mode-btn--active' : ''}"
          ...
          <svg width="13" height="13" viewBox="0 0 16 16" fill="none" aria-hidden="true">
            ...
            <path d="M10 1h7l3 3v11h-10" stroke="none"/>
          </svg>
          Individual PDFs
        </button>
  ...
  panel.innerHTML = `
    <div class="jpg-queue-header">
      <span class="jpg-queue-badge">
        ...
        <span class="jpg-queue-badge-text" id="jpg-queue-hint">
          <strong>${total}</strong> image${total !== 1 ? 's' : ''} &nbsp;·&nbsp; ${outHint}
        </span>
      </span>
      <button class="jpg-queue-clear-btn" id="jpg-queue-clear" type="button">Clear all</button>
    </div>
    ...
    <div class="jpg-queue-actions">
      <input class="jpg-queue-filename" id="jpg-queue-filename"
             type="text" placeholder="Output filename (optional)"
             value="${_esc(defaultName)}" maxlength="120" spellcheck="false"/>
      <span class="jpg-queue-ext" id="jpg-queue-ext">.${extHint}</span>
      <button class="jpg-queue-convert-btn" id="jpg-queue-convert" type="button">${label}</button>
    </div>`;
  ...
  if (isPdf && isMulti) {
    panel.querySelectorAll('.jpg-pdf-mode-btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        // Update hint text
        const hintEl = panel.querySelector('#jpg-queue-hint');
        const extEl  = panel.querySelector('#jpg-queue-ext');
        if (hintEl) {
  ...
  panel.querySelector('#jpg-queue-clear').addEventListener('click', () => {
    removeJpgPanel();
    const zone = document.getElementById('drop-zone');
    if (zone) {
  ...
  panel.querySelector('#jpg-queue-convert').addEventListener('click', () => {
    if (_queue.length < 1) return;
    const inp     = panel.querySelector('#jpg-queue-filename');
...
async function _addFiles(fileArray) {
  const jpgFiles = fileArray.filter(_isJpg);

  if (jpgFiles.length < fileArray.length) {
    pushNotification({
      type: 'warning',
      message: 'Some files skipped — only .jpg / .jpeg files are accepted.',
    });
  }
...
export async function handleJpgFilesPicked(files, toolId) {
  _toolId = toolId;
...
async function _submitConvert(outputFilename) {
  ...
  // Snapshot queue + toolId before teardown
  ...
  const pdfModeCopy   = _pdfMode;  // capture before reset
  // For jpg-pdf single mode with multiple images → output is a .pdf, otherwise .zip
  const earlyExt      = (toolId === 'jpg-pdf' && isMulti && pdfModeCopy === 'single')
    ? 'pdf'
    : (isMulti ? 'zip' : ext);
  ...
  // Send pdf_mode only for the jpg-pdf tool
  if (toolId === 'jpg-pdf') fd.append('pdf_mode', pdfModeCopy);
  ...
  let jobId;
  try {
    const res  = await fetch(`${BACKEND}/api/jpg/${route}/convert`, { method: 'POST', body: fd });
